...
document.getElementById("cadastroForm").addEventListener("submit", async (e)=>{
  ...
  try{
    ...
    msg.textContent = "Configurando sua conta...";
    const slug = (nomeNegocio.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "medico") + "-" + Math.random().toString(36).slice(2,6);
    const { data: business, error: bizError } = await supabaseClient.from("businesses")
      .insert({ owner_id: session.user.id, slug, name: nomeNegocio || "Meu LaudoPro", segment: "laudos", subscription_plan: planoSelecionado })
      .select().single();
    if(bizError){ msg.textContent = "Erro ao configurar a conta: " + bizError.message; btn.disabled = false; return; }

    msg.textContent = "Abrindo o pagamento...";
    const resp = await fetch(`${BACKEND_URL}/api/assinatura/criar`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ business_id: business.id, email, plano: planoSelecionado, produto: "laudopro" })
    });
